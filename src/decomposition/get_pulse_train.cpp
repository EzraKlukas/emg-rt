...
namespace {
    ...
    using ConstEigenMatrixMap =
        Eigen::Map<const EigenMatrix, Eigen::Unaligned>;

    using EigenMatrixMap =
        Eigen::Map<EigenMatrix, Eigen::Unaligned>;

    inline Eigen::Index ei(std::size_t x) {
...
void get_pulse_train(RingMatrix<float> &pulse_t,
    ...
    // index = col * rows + row.
    ConstEigenMatrixMap W(
            mu_filters.data_handle(),
            ei(filters),
    ...
    while (done < samples) {
        const std::size_t x_phys_col =
            (emg_buffer.head + done) % emg_buffer.cols;

        const std::size_t y_phys_col =
            (pulse_t.head + done) % pulse_t.cols;

        const std::size_t x_cols_until_wrap =
            emg_buffer.cols - x_phys_col;

        const std::size_t y_cols_until_wrap =
            pulse_t.cols - y_phys_col;
        ...
        ConstEigenMatrixMap X(
                &emg_buffer.data[x_phys_col * extended_channels],
                ei(extended_channels),
                ei(chunk));

        EigenMatrixMap Y(
                &pulse_t.data[y_phys_col * filters],
                ei(filters),
                ei(chunk));
